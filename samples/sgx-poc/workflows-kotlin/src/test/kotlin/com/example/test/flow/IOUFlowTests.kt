...
class IOUFlowTests {
    ...
    fun `flow rejects invalid IOUs`() {
        ...
        // The TheContract specifies that IOUs cannot have negative values.
    ...
    fun `recorded transaction has no inputs and a single output, the input IOU`() {
        ...
        for (node in listOf(a, b)) {
            ...
            val recordedState = txOutputs[0].data as SecretDeal
    ...
    fun `flow records the correct IOU in both parties' vaults`() {
        ...
        for (node in listOf(a, b)) {
            node.transaction {
                val ious = node.services.vaultService.queryBy<SecretDeal>().states
