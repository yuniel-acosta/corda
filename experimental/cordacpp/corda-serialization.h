#ifndef CORDA_SERIALIZATION_H
#define CORDA_SERIALIZATION_H
...
#include <cxxabi.h>   // For name demangling in error message.
...
class Any {
public:
    virtual ~Any() = default;   // Must have at least one virtual function to force vtable generation.
};

/**
 * A reference counting pointer.
 *
 * We define a custom name for std::shared_ptr so that we can distinguish in the type system fields that are meant to
...
/**
 * Represents each element of the binary schema language that's embedded in each message.
 */
...
class Parser {
public:
    // Keep the parser around in a TLS slot during deserialisation so it's usable in the operator overloads.
    // We use op overloads for consistency, as that's how the Proton API works. It simplifies code generation
    // to only have one syntax to worry about.
    ...
    ptr<T> parse() {
        ...
        ptr<T> t;
        // This line may throw an exception, in which case 'current' can end up being a dangling pointer. It should be
        // safe as we won't use it until reinitialisation.
        decoder >> t;
...
template <class T> inline ptr<T> parse(const std::string &bytes) { return Parser(bytes).parse<T>(); }
...
proton::codec::decoder &operator>>(proton::codec::decoder &decoder, ptr<T> &out) {
    ...
    decoder >> block;
    std::string classname = Parser::current->lookup(sym);
    ...
    // Construct the object given the type in the stream.
    auto result = (*ctor)(decoder);
    // Attempt to cast to the expected type. If the developer didn't get the class they expected, this will yield null.
    out = net::corda::ptr<T>(dynamic_cast<T*>(result));
    if (out == nullptr) {
        int status;
