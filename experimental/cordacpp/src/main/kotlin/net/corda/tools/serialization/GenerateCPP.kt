...
class GenerateCPPHeaders : CordaCliWrapper("generate-cpp-headers", "Generate source code for reading serialised messages in C++ languages") {
    ...
    // Result of generating a file for a class. Contains the generated text, along with other types referred to in the
    // definition of this type that we might need to recursively generate.
    ...
    private fun generateClassFor(type: Type, seenSoFar: Set<String>): GenResult? {
        ...
        val supersPre = typeInformation.interfacesOrEmptyList.toMutableList()
        typeInformation.superClassOrNull?.let { supersPre += it }
        val supers: List<LocalTypeInformation> = supersPre.filterNot {
    ...
    private fun convertType(resolved: Type, genericReturnType: Type?): Pair<String, Set<Type>> {
        ...
        val cppType = when (resolved.typeName) {
            ...
            // Mapped using CustomSerializer.ToString
            "java.math.BigDecimal" -> "std::string"
            "java.util.Currency" -> "std::string"
