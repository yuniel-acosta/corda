...
internal data class LocalTypeInformationBuilder(val lookup: LocalTypeLookup,
    ...
    fun buildReadOnlyProperties(rawType: Class<*>): Map<PropertyName, LocalPropertyInformation> =
...
fun Method.returnsNullable(): Boolean = try {
...
//    throw RuntimeException("Mystery with ${this.declaringClass.name}/${this.name}")
//}
...
private fun constructorForDeserialization(type: Type): KFunction<Any>? {
    ...
    val kotlinCtors = try {
        clazz.kotlin.constructors
    } catch (e: RuntimeException){
        throw java.lang.RuntimeException("on class" + clazz.name)
    }
