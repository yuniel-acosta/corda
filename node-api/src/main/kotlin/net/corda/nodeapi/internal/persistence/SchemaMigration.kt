...
open class SchemaMigration(
        ...
        private val netParametersPath: Path?,
    ...
    companion object {
        ...
        const val NODE_NET_PARAMS_PATH_KEY = "liquibase.nodeNetParamsDir"
    ...
    protected fun prepareResources(schemas: Set<MappedSchema>, forceThrowOnMissingMigration: Boolean): List<Pair<CustomResourceAccessor, String>> {
        ...
        val path = netParametersPath?.toString()
        if (path != null) {
            System.setProperty(NODE_NET_PARAMS_PATH_KEY, path) // base dir for any custom change set which may need to load a file (currently AttachmentVersionNumberMigration)
        }
        if (ourName != null) {
